# W01 — Visual / Scientific Product Design — RC.6 Ready-to-Paste GPT Work Prompt
...
这是一个 statistical research IDE / model architecture atlas，不是 SaaS dashboard。请从普通研究者真实看到的界面判断是否达到可以进入最终人工验收的视觉与信息设计水平。

重点验收：
1. 首屏的信息层级是否清楚：Asteria / Project / View / Model / central canvas / right inspector 谁主谁次。
2. Architecture 默认 `Overview` 是否真的无需先缩放就能读懂主要科学骨架；再切 `Full model`，确认全图仍完整但密度是用户主动选择的。
3. 1536×864 与 1366×768 都检查 CAT-TRACE Overview；核心 symbol、node label、edge、lane heading 和 selected context 是否清晰。
4. 检查数学显示：核心符号、公式、定义不应出现 raw LaTeX、不可识别截断或过小排版。
5. 检查默认 `TRACE OFF` 状态与显式 Show trace 后的差异；selected、upstream、downstream、muted context 是否形成清楚视觉层级。
6. 关系边是否能被人跟踪：方向、粗细、arrow/line treatment、交叉、active vs unrelated edge 对比是否合理。
7. Right inspector 是否 researcher-first：Meaning / Why it matters / definition / relation context 应先于 advanced/debug；核心公式是否真正数学排版。
8. Light / Dark 都检查。Light 不能把 muted node/edge/label 洗成水印；Dark 保持 Quiet Celestial、不过度炫技。
9. Architecture / Lineage / Evidence 保持统一品牌，但三种认知任务是否一眼可区分。
10. 检查产品是否仍暴露明显工程/debug 语言、internal schema/fixture 术语、raw JSON 等到主理解路径。
11. 判断当前实现与成熟 scientific IDE 相比最明显的 5 个视觉缺口；不要因为“可以用”就放低 stable 前标准。

至少保存/描述这些状态的截图证据：
- CAT-TRACE Overview dark 1536×864
- CAT-TRACE Overview dark 1366×768
- CAT-TRACE Overview light 1366×768
- Full model
- trace OFF vs trace ON focus
- Original TRACE
- Lineage
- Evidence

Severity：P0/P1/P2/P3。P1 仅用于核心 scientific map 在正常目标 viewport 下仍无法可靠阅读、核心公式/结构不可辨，或主要产品界面严重破坏。
...
正文必须有 Executive summary / Coverage / Findings / Positive observations / Not tested / Browser execution note / Top fixes before stable。每个 finding 给 Start state、Steps、Observed、Expected、Impact、Evidence、Reproducibility。
````
