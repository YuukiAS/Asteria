# W04 — RC.10 First-time Researcher UX — Ready-to-Paste GPT Work Prompt

````text
你是第一次使用 Asteria 2.0 的统计/生物统计研究者，做独立 UI 黑箱验收。

目标：https://asteria.httpwwwcardiacnexus-ukb.com/
预期版本：2.0.0-rc.10

不要读取 repo、源码、旧报告、旧截图、实现说明。不要假设开发者在旁边解释。只通过真实页面判断：一个第一次使用的研究者能不能独立理解并愿意继续使用。
...
本轮还必须执行以下 visual/product-quality gate：

- 先看整屏 gestalt，不要只做 checklist；
- Architecture/Lineage/Evidence 如果出现 P2 级 visual/readability 问题，必须 FAIL，不允许 PASS+P2；
- 主阅读层数学必须正规渲染，不能出现 raw `beta^`, `gamma_`, `a_g`, `p_g^*`, `mathcal` 等；
- stable-facing copy 不能是 schema/debug/internal/AI-template 口吻；
- Lineage relation 必须简洁、克制、归属清楚；
- Evidence 的 Why it matters / gaps 必须 entity-specific，而不是 graph-topology 模板；
- Advanced/debug 不应污染默认主阅读路径。

## 任务

请像一个第一次使用者一样，不看说明书，实际操作 10–15 分钟。

1. 首屏判断：你能否解释 Asteria 是什么、Project/View/Model 分别是什么？
2. Architecture Overview：能否读出 data -> matching/group -> latent -> parameterization -> inference/target 的故事？
3. 点击 `Open-tail slope`：Meaning / Why it matters / canonical definition 是否真的帮助理解？数学是否像论文而不是代码？
4. 切 Full model：虽然更密，但是否仍像“可深入查看”的地图，而不是卡片堆叠？
5. 切 Original TRACE 再回 CAT-TRACE：是否能理解两者差异？
6. 看 Semantic Diff 至少 6 项：每项 Why it matters 是否具体，还是换词后的统一模板？
7. Lineage：第一眼能否看出 TRACE/HMSC/bigMVP/MGP 到 CAT-TRACE 的关系？connector/chip 是否自然，还是像 generic graph/debug network？
8. Evidence：点击 claim、proof、dataset、implementation、limitation/pending theorem；Why it matters / What is missing / What would close this 是否真正对应当前对象？
9. 搜索 `HMSC`、`Finland`，看跨 view 导航是否自然。
10. Advanced / Export 默认是否不干扰主阅读；关闭状态下 raw JSON/schema/warnings 是否真的不可见。
11. 回答：如果没有开发者在旁边，你是否愿意第二次打开 Asteria 来判断模型结构/方法谱系/证据状态？
...
正文请列出：
- 5 个最严重 comprehension breakdown（不足 5 个就不要硬凑）；
- 3 个最有价值功能；
- Would I Open It Again；
- Executive Summary / Coverage / Findings / Positive Observations / Not Tested / Browser Execution Note / Top Fixes Before Stable。

核心 Architecture/Lineage/Evidence 如果存在 P2 级可读性/成品问题，AUDIT_RESULT 必须 FAIL。
````
