# W05 — RC.7 Responsive / Accessibility — Ready-to-Paste GPT Work Prompt
...
这是 RC.7 的 targeted re-audit。不要读取 Asteria repo、旧审计报告或旧截图。
...
本轮只针对 RC.7 改过的 accessibility / responsive surface 做 hard re-audit。

必须测试：

A. Fresh keyboard entry
- 新开 fresh page/profile；
- 从页面内容开始按 Tab；
- 第一、第二个页面级 tab stops 应为 Skip to canvas / Skip to inspector；
- focus 时 skip link 必须真实可见；
- Enter 激活后 focus 分别落到 canvas / inspector；
- 不要把 browser chrome 算入页面 tab order。

B. 1366×768 Light trace state
- CAT Architecture Overview；
- Light theme；
- 开启 trace；
- muted nodes、edges、关系标签仍必须可辨；
- selected/upstream/downstream 有非纯颜色 cue；
- 不允许 context 淡到接近水印。

C. Compact top actions
- 1366 宽度检查 Search / Export / Save / Restore / inspector / theme；
- accessibility-visible name 必须明确；
- focus ring 清楚；
- 点击/聚焦区域不能小得明显难用，目标约 40px 量级；
- 不发生 header 横向 overflow。

D. Full model reading controls
- 切 Full model；
- 找到 Zoom out / Fit / Zoom in；
- keyboard 至少可操作 zoom buttons；
- Zoom in 后尝试 pointer drag pan；
- Fit 应恢复可理解的 canonical fit state；
- controls 必须可发现、有 accessible name；
- Overview 返回后不应残留 stale Full-model transform。

E. Basic responsive smoke
- 1536×864 Overview / Full model；
- 1366×768 Overview / Full model；
- inspector collapse/expand；
- 不应出现 page-level horizontal scroll trap。

125% browser zoom 如果当前真实 browser UI 可正常设置则测试；若工具无法做到，明确 NOT_TESTED，不因此 block。
...
正文必须明确列出 A-E 每个场景的实际测试结果，并包含 Executive summary / Coverage / Findings / Positive observations / Not tested / Browser execution note / Top fixes before stable。
````
