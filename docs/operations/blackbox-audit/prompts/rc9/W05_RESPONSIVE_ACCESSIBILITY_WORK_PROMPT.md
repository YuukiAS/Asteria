# W05 — RC.9 Responsive / Accessibility / Dense Scientific UI — Ready-to-Paste GPT Work Prompt
...
这是 RC.9 broad visual repair 后的 fresh re-audit。不要读取 Asteria repo、旧报告、旧截图或实现说明。只通过普通用户真实 UI 判断。
...
## Visual Acceptance hard rule

本轮不只是 accessibility checklist。必须同时做整屏视觉审查：
- node/card overlap；
- edge/relation label 是否盖住 card；
- primary symbol/title clipping；
- 选择不同节点后的 shared geometry stability；
- 1366/1536 Light/Dark 长时间阅读；
- Lineage/Evidence card 是否贴边、拥挤、relation text 是否像裸大字压线。

如果核心 Architecture/Lineage/Evidence 在 required desktop viewport 上存在 P2 级视觉/可读性问题，本 reviewer 必须 FAIL/FIX_THEN_RETEST，不允许 PASS + P2。

## 必须测试

A. 1536×864 Architecture Overview Dark
- 整屏检查 card spacing、lane、relation path、inspector balance；
- primary symbols/text 完整。

B. 1366×768 Architecture Overview Light
- trace OFF；
- selected `Open-tail occurrence`；
- selected `gamma_g`；
- `beta^U_gh` trace ON；
- 检查 node overlap / label collision / muted context / clipping。

C. Geometry stability
- 连续选择 `y^U_igh -> gamma_g -> beta^U_gh -> c(f)`；
- shared nodes 不应明显跳位；
- reveal context 不应挤压/覆盖既有 cards。

D. Keyboard
- fresh page first Tab/skip links；
- model selector、search、view controls、Show trace、Full model/Overview、Zoom/Fit controls、Advanced disclosure；
- focus ring 清楚；
- 不应因 graph nodes 数量导致无法合理到达关键 controls。

E. Full model 1366/1536
- Zoom in / pan / Fit；
- primary symbol/title 不得因 fit 密度而被邻卡/label 遮住；
- 回 Overview 无 stale transform。

F. Lineage 1536 + 1366
- source cards/target card 不贴边；
- relation text 不横跨整屏、不卡在 card 上；
- selected state可辨。

G. Evidence 1536 + 1366
- claim/proof/dataset/implementation/gap cards 不 overlap；
- pending/support/limited 标签/线可辨；
- 不出现长 relation text 压图。

H. Inspector / trace controls 1366
- 不出现无意义 scroll strip；
- chips/counters/Clear 分组整洁；
- Advanced disclosure 不破坏 scroll/focus。

I. Light/Dark
- Architecture/Lineage/Evidence 各 smoke；
- light muted context 可读，dark 不过重。

125% browser zoom 如果真实 browser UI 可正常设置则测试；若工具无法做到，明确 NOT_TESTED，不因此 block。
