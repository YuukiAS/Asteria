# W02 — RC.9 Statistical Semantics — Ready-to-Paste GPT Work Prompt
...
这是 RC.9 broad visual/product repair 后的 fresh semantics regression audit。不要读取 repo、旧报告、旧截图或实现说明。所有判断来自真实 UI；expected scientific invariants 仅以本 prompt 内提供的内容为准。
...
最终产品判断必须来自真实 staging 网站、浏览器正常渲染、普通用户可以看到/操作的前端 UI。允许导航、click、fill、select、check、keyboard、scroll、refresh、Back/Forward、截图和正常页面状态观察。
## 4. 严禁绕过 UI
...
## 6. Screenshot 证据
截图必须来自真实 staging 页面渲染结果。可以由 in-app Browser、Chrome、Edge、Chromium、Playwright 等生成。
## 7. Browser timeout 与产品 bug 分离
click timeout、fill timeout、navigation wait timeout、Browser handle lost、accessibility snapshot timeout 首先是测试工具问题。必须重新观察真实页面，只有页面本身确实异常才能记录产品缺陷。
...
```
只有读取普通用户无法看到的内部实现信息并影响判断时才设 YES。
## 9. 真正的 Browser blocker
只有 in-app Browser 无法工作 AND 合理真实-browser fallback 也无法工作 AND 无法通过任何真实浏览器继续 consumer UI 时，才 `BLOCKED_BY_BROWSER_ENVIRONMENT`。
## 10. 最终判据
每一个 finding 只问：这个现象是否能够仅通过真实网站用户界面被观察或触发？YES 才可作为黑箱证据。
...
- `beta_j ~ N_q(nu,Psi)`；
- 不包含 finite catalogue `K`、`c(f)`、grouped open tail `g/a_g/pi_g`、catalogue/open-tail discovery split、`nu_g`。
...
- CAT-TRACE 不是机械的 TRACE + HMSC merge。

## 必须测试

1. Model switch Original TRACE -> CAT-TRACE -> Original TRACE -> CAT-TRACE，检查 central graph / inspector / diff 不串 model。
2. CAT-TRACE 检查 `mathcal K`, `mathcal U`, `c(f)`, `beta^U_gh`, `gamma_g`, `p_g`, `alpha^U_gh`, `Sigma_W`, `nu`, `pi_g`, `p_g^*`。
3. 检查 `c(f)=empty -> mathcal U` visible relation/explanation。
4. 检查 indexed metadata 至少 `beta^U_gh/alpha^U_gh` 为 `g,h`，`gamma_g/pi_g/p_g/p_g^*` 为 `g`。
5. Semantic Diff：Added/Changed/Preserved 的 scientific claims 必须正确，且数学显示不能改变含义。
6. Lineage：extends/preserves/borrows/computational inspiration/methodological component 关系不能变成错误 claim。
7. Evidence：三个 first-paper datasets 仍 pending；real-data closure 是 limitation；marked discovery theorem 是 Pending；implementation evidence 不冒充 theorem proof。
8. 检查 RC.9 visual/copy cleanup 没有为了“说人话”而改错 scientific truth。
...
正文：Executive summary / Coverage / Findings / Positive observations / Not tested / Browser execution note / Top fixes before stable。
````
