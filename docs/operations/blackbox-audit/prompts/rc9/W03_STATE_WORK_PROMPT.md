# W03 — RC.9 Interaction / State Coherence — Ready-to-Paste GPT Work Prompt

````text
你是 Asteria 2.0 的独立 interaction / state-coherence 黑箱 reviewer。
...
这是 RC.9 broad visual repair 后的 fresh re-audit。不要读取 Asteria repo、旧审计、旧截图或实现说明。只通过真实 UI 测试状态一致性，并特别验证 selection/trace 不再让 shared graph geometry 漂移。
...
## 本轮 hard state checks

A. Default
- fresh page：CAT-TRACE / Architecture / Overview / trace OFF；
- selected item 与 inspector 一致；没有 stale status。

B. Geometry stability under selection
- 在 Overview 连续选择：Open-tail occurrence -> Group open-tail intensity -> Open-tail slope -> Catalogue match。
- 观察共享节点是否保持稳定位置；允许 reveal 新节点，但已有 shared nodes 不应整体重新排布或滑动。
- 如果 UI automation 能读取 visible element bounding rect，可用它作为用户可见几何的辅助证据；不得读取隐藏 state。
- 视觉上明显 reflow / jump 视为产品 finding。

C. Trace lifecycle
- select `beta^U_gh` -> Show trace -> Direct/Both；
- Recursive depth 3，Both -> Upstream -> Downstream；
- Clear/Reset：trace 必须 OFF、counter/path/highlight 清理，selection/inspector 回到明确默认；
- 再开启 trace，不能残留旧 reveal/context。

D. Model/detail state
- CAT Overview -> Full model -> Zoom/Pan/Fit -> Overview；
- Original TRACE -> CAT-TRACE；
- 不得带入错误 transform、隐藏节点、trace 或 inspector state。

E. View state
- Architecture -> Lineage -> Evidence -> Architecture，连续 3 轮；
- cross-view link 至少一次；
- right inspector type/title/selected entity 必须跟随 view。

F. Search
- Current view / All graph；
- HMSC、Finland、beta/open-tail symbol、no-result 字符串；
- cross-view jump 之后 state 不串台。

G. Export/session/theme
- Advanced disclosure、top Export、Save/Restore、Light/Dark、refresh；
- 不要求 theme 必须随 Save/Restore 恢复（除非 UI 自己声称如此）；
- model/view/detail/trace 核心 session 必须恢复。

H. Motion quality
- 连续点击至少三个节点，观察 motion；
- graph geometry 不应出现大范围动画滑动；
- 轻量 opacity/border/background/stroke transition 可以接受。

## 额外结果字段

```text
SELECTION_GEOMETRY_STABLE = PASS | FAIL
TRACE_LIFECYCLE_COHERENT = PASS | FAIL
VIEW_MODEL_STATE_COHERENT = PASS | FAIL
