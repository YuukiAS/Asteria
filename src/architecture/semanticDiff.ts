...
export function diffOriginalTraceToCatTrace(_original: ArchitectureProjectV2, _cat: ArchitectureProjectV2): SemanticDiffReport {
  return {
    ...
    items: [
      { id: "diff:finite-catalogue", status: "added", label: "Finite catalogue identities", labelParts: [{ text: "Finite catalogue " }, { latex: "\\mathcal K" }, { text: ", " }, { latex: "K" }, { text: ", and " }, { latex: "\\mathcal K_n" }], after: "CAT-TRACE separates finite catalogue identities from catalogue-external open tail.", why: "This makes the observed catalogue boundary explicit before any open-tail discovery claim." },
      { id: "diff:matching", status: "added", label: "Deterministic catalogue matching", labelParts: [{ text: "Deterministic " }, { latex: "c(f)" }, { text: " matching" }], after: "Raw features are matched to catalogue identities or open tail.", why: "The graph now shows how raw detections become catalogue or open-tail observations." },
      { id: "diff:grouping", status: "added", label: "Biological group deviation", labelParts: [{ text: "Biological grouping with " }, { latex: "a_g" }], after: "Open tail and catalogue responses share biological group structure with sum-to-zero group deviation.", afterParts: [{ text: "Open tail and catalogue responses share biological group structure with sum-to-zero " }, { latex: "a_g" }, { text: "." }], why: "Group structure is visible without changing the marginal-response interpretation." },
      { id: "diff:open-tail-slots", status: "added", label: "Grouped truncation and zero slots", labelParts: [{ text: "Grouped " }, { latex: "p_g" }, { text: ", " }, { latex: "p_g^*" }, { text: ", and zero slots" }], after: "p_g is fixed truncation; zero slots remain likelihood information.", afterParts: [{ latex: "p_g" }, { text: " is fixed truncation; zero slots remain likelihood information." }], why: "This prevents the interface from treating anonymous zero slots as empty rows." },
      { id: "diff:gamma-composition", status: "added", label: "Group open-tail intensity", labelParts: [{ latex: "\\gamma_0" }, { text: ", " }, { latex: "\\pi_g" }, { text: ", and " }, { latex: "\\gamma_g" }], after: "gamma_g = gamma_0*pi_g is derived group intensity.", afterParts: [{ latex: "\\gamma_g=\\gamma_0\\pi_g" }, { text: " is derived group intensity." }], why: "The derived intensity is shown as a composition, not as a free intercept or group effect." },
      { id: "diff:catalogue-slope", status: "added", label: "Catalogue intercept/slope structure", after: "Catalogue species receive finite intercepts and optional trait/relatedness modules.", why: "Finite catalogue species stay separated from anonymous open-tail parameterization." },
      { id: "diff:discovery-split", status: "added", label: "Catalogue/open-tail discovery split", after: "Targets distinguish finite catalogue discovery from open-tail discovery.", why: "The target layer now exposes which discovery quantity each path supports." },
      { id: "diff:factor-copula", status: "added", label: "Normalized finite-working-set residuals", labelParts: [{ text: "Finite-working-set normalized factor-copula" }], after: "Residual dependence is scoped to mathcal W with diag(Sigma_W)=1.", afterParts: [{ text: "Residual dependence is scoped to " }, { latex: "\\mathcal W" }, { text: " with " }, { latex: "\\operatorname{diag}(\\Sigma_{\\mathcal W})=1" }, { text: "." }], why: "Residual dependence is kept inside a finite working set while preserving marginal means." },
      { id: "diff:slope", status: "modified_definition", label: "Species response hierarchy", before: "beta_j ~ N_q(nu,Psi)", beforeParts: [{ latex: "\\beta_j\\sim N_q(\\nu,\\Psi)" }], after: "beta^U_gh = nu + a_g + v^U_gh", afterParts: [{ latex: "\\beta^{\\mathcal U}_{gh}=\\nu+a_g+v^{\\mathcal U}_{gh}" }], why: "CAT-TRACE keeps the shared response vector and adds group plus species-level open-tail structure." },
      { id: "diff:target", status: "modified_target", label: "Richness/discovery target", before: "TRACE calibrated finite richness target", after: "CAT-TRACE catalogue/open-tail discovery decomposition", why: "The target moves from one calibrated richness view to a catalogue/open-tail decomposition." },
