...
try {
  ...
  for (let run = 0; run < 30; run += 1) {
    ...
    start = performance.now()
    projectLayerFocus(project, "view:architecture", "parameterization")
    ...
    JSON.stringify(
      {
        ...
        relationCount: Object.keys(project.relations).length,
        visibleProjectionCount: project.views["view:architecture"].projectedEntityIds.length,
        indexAverageMs: Number(average(indexRuns).toFixed(3)),
        traceAverageMs: Number(average(traceRuns).toFixed(3)),
        layerFocusAverageMs: Number(average(layerRuns).toFixed(3)),
      },
      null,
      2,
    ),
